...
import { EliminarRegistro } from "../pacientes/[id]/EliminarRegistro";
...
export default async function AnalisisPage({ searchParams }: { searchParams: Promise<{ tab?: string; page?: string; q?: string }> }) {
  ...
  return <div>
    <div className="mb-4"><div className="flex flex-wrap items-center justify-between gap-2"><h1 className="text-xl font-semibold text-slate-800">Análisis</h1><Link href="/dashboard/pacientes"><Button size="sm">+ Nuevo análisis</Button></Link></div><p className="text-xs text-slate-500">{total.toLocaleString("es-AR")} registros</p></div>
    ...
    <div className="space-y-2 md:hidden">{rows.map((row, index) => { const patientId = String(row[PATIENT_ID[activeTab]] ?? ""); const ownerId = ownerFor(patientId); const deleteTipo = activeTab === "orina" ? "orina" : activeTab === "quimica" ? "quimica" : "hemograma"; const deleteId = Number(row[activeTab === "orina" ? "ori_id" : activeTab === "quimica" ? "qs_id" : "hem_id"]); return <article key={index} className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-2"><Link href={`/dashboard/pacientes/${patientId}`} className="block truncate font-semibold text-slate-800 hover:text-blue-700">🐾 {patientNames[patientId] || `Paciente #${patientId}`}</Link><EliminarRegistro tipo={deleteTipo} id={deleteId}/></div><p className="mt-1 text-xs text-slate-500">{String(row[DATE[activeTab]] ?? "").trim() || "Sin fecha"} · Dr/a: {String(row[DOCTOR[activeTab]] ?? "").trim() || "—"}</p>{ownerId && <Link href={`/dashboard/clientes/${ownerId}`} className="mt-1 block truncate text-xs text-blue-700 hover:underline">👤 {ownerNames[String(ownerId)] || "Ver dueño"}</Link>}<Metrics values={visibleMetrics(activeTab, row)}/><Details row={row}/></article>; })}{!rows.length && <p className="rounded-xl border bg-white p-8 text-center text-slate-400">Sin resultados</p>}</div>
    <div className="hidden overflow-x-auto rounded-xl border bg-white shadow-sm md:block"><table className="w-full text-sm"><thead className="border-b bg-slate-50"><tr><th className="px-4 py-2.5 text-left font-medium text-slate-600">Paciente</th><th className="px-4 py-2.5 text-left font-medium text-slate-600">Dueño</th><th className="px-4 py-2.5 text-left font-medium text-slate-600">Fecha</th><th className="px-4 py-2.5 text-left font-medium text-slate-600">Médico</th>{visibleMetrics(activeTab, {}).map(([name]) => <th key={String(name)} className="px-4 py-2.5 text-left font-medium text-slate-600">{String(name)}</th>)}<th className="px-4 py-2.5 text-left font-medium text-slate-600">Detalle</th><th className="px-4 py-2.5 text-left font-medium text-slate-600">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((row, index) => { const patientId = String(row[PATIENT_ID[activeTab]] ?? ""); const ownerId = ownerFor(patientId); const deleteTipo = activeTab === "orina" ? "orina" : activeTab === "quimica" ? "quimica" : "hemograma"; const deleteId = Number(row[activeTab === "orina" ? "ori_id" : activeTab === "quimica" ? "qs_id" : "hem_id"]); return <tr key={index} className="hover:bg-slate-50"><td className="px-4 py-2.5 text-xs"><Link href={`/dashboard/pacientes/${patientId}`} className="text-blue-700 hover:underline">🐾 {patientNames[patientId] || `#${patientId}`}</Link></td><td className="px-4 py-2.5 text-xs">{ownerId ? <Link href={`/dashboard/clientes/${ownerId}`} className="text-blue-700 hover:underline">{ownerNames[String(ownerId)] || "—"}</Link> : "—"}</td><td className="px-4 py-2.5 text-xs text-slate-500">{String(row[DATE[activeTab]] ?? "").trim() || "—"}</td><td className="px-4 py-2.5 text-xs text-slate-500">{String(row[DOCTOR[activeTab]] ?? "").trim() || "—"}</td>{visibleMetrics(activeTab, row).map(([name, value]) => <td key={String(name)} className="px-4 py-2.5 text-xs">{String(value ?? "").trim() || "—"}</td>)}<td className="px-4 py-2.5"><Details row={row}/></td><td className="px-4 py-2.5"><EliminarRegistro tipo={deleteTipo} id={deleteId}/></td></tr>; })}{!rows.length && <tr><td colSpan={10} className="px-4 py-8 text-center text-slate-400">Sin resultados</td></tr>}</tbody></table></div>
