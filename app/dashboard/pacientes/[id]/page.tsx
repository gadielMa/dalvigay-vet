...
export default async function PacienteDetailPage({
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div><Link href={`/dashboard/turnos?cliente_id=${paciente.pac_cliente}&paciente_id=${id}`}><Button variant="outline" size="sm">🗓️ Agendar turno</Button></Link><PacienteActions paciente={paciente} clienteId={Number(paciente.pac_cliente)} /><BajaPaciente id={Number(id)} nombre={paciente.pac_nombre?.trim() || "este paciente"} inactivo={paciente.pac_fecha_des?.trim() === "Inactivo"} /><NuevaConsulta pacienteId={Number(id)} /></div>
